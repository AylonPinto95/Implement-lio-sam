      ...
      initialized_(false)
{
    ...
    imu_subscriber_.reset(new ImuSubscriber(this, "os_node/imu", qos));
...
void ImuFilterMadgwickRos::checkTopicsTimerCallback()
{
    if (use_mag_)
        RCLCPP_WARN_STREAM(
            get_logger(),
            "Still waiting for data on topics /os_node/imu and /imu/mag...");
    else
        RCLCPP_WARN_STREAM(get_logger(),
                           "Still waiting for data on topic /os_node/imu...");
}
